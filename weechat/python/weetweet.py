...
def dict_tweet(tweet_id):
    ...
    if not tweet_id in tweet_dict.values():
        ...
        if cur_index[-1] == '9':
            if cur_index[1] == 'z':
                cur_index = chr(ord(cur_index[0]) + 1) + 'a0'
            else:
                cur_index = cur_index[0] + chr(ord(cur_index[1]) + 1) + '0'
        else:
            cur_index = cur_index[:-1] + chr(ord(cur_index[-1]) + 1)
...
def stream_message(buffer,tweet):
    if 'delete' in tweet:
        ...
        pass # actually, I don't want a wall of junk every time I fav a tweet
...
def create_stream(name, args = ""):
    ...
    proc_hooks[name] = weechat.hook_process("python3 {} stream {} {}"
...
def my_process_cb(data, command, rc, out, err):
    ...
    if rc == weechat.WEECHAT_HOOK_PROCESS_ERROR:
        weechat.prnt("", "Error with command '%s'" %
                command.replace(script_options["oauth_token"],"<token>").replace(script_options["oauth_secret"],"<secret>"))
...
def get_twitter_data(cmd_args):
    ...
    try:
        if cmd_args[-1][0] == "[":
            option_list = json.loads(cmd_args[-1])
            cmd_args.pop(-1)
...
def oauth_proc_cb(data, command, rc, out, err):
    ...
    """)
            ...
            weechat.hook_process("python3 {} {} {} settings []"
                                 .format(SCRIPT_FILE_PATH,
                                         script_options["oauth_token"],
                                         script_options["oauth_secret"]),
                                 10 * 1000,
...
def oauth_dance(buffer, pin = ""):
    ...
    if pin == "":
        ...
        weechat.hook_process("python3 {} '' '' auth".format(SCRIPT_FILE_PATH),
                             10 * 1000, "oauth_proc_cb", "auth1")
    else:
        oauth_verifier = pin.strip()
        weechat.hook_process("python3 {} {} {} auth {}"
                             .format(SCRIPT_FILE_PATH,
                                     script_options["oauth_token"],
                                     script_options["oauth_secret"],
                                     oauth_verifier),
...
def finish_init():
    ...
    weechat.hook_process("python3 {} {} {} f {} []"
                         .format(SCRIPT_FILE_PATH,
                                 script_options["oauth_token"],
                                 script_options["oauth_secret"],
                                 script_options["screen_name"]),
                         10 * 1000,
                         "oauth_proc_cb",
                         "friends")
